...
void Button::Draw(SpriteRenderer& renderer)
{
    ...
    if (isRect) {
        renderer.DrawRectangle(this->Position, glm::vec2(this->Width, this->Height), this->Rotation, drawColor);
        return;
    }
    renderer.DrawSprite(this->Sprite, this->Position, glm::vec2(this->Width, this->Height), this->Rotation, drawColor);
}